            ...
            if (err) {
                if (err.code == 11000) {
                    ...
                    res.status(400)
                        .json({
                            ppResult: 'err',
                            ppMsg: err.ppMsg ? err.ppMsg : '注册失败!',
                            err: err
                        });
                }
            } else {
...
router.post('/getLastLocation', function (req, res) {
    res.json({
        ...
        ppData: {
            ...
            lastLocationTime: req.user.lastLocationTime
        }
    });
});
...
router.post('/confirmMeetSearchTarget', function (req, res) {
    ...
    async.parallel([
            ...
            function (callback) {
                ...
                Meet.findOne({
                        _id: req.body.meetId,
                        createrUsername: req.user.username,
                        status: '待确认'
                    })
                    .exec(function (err, doc) {
                        if (err) {
                                ...
                                function (err, doc) {
                                    if (err) {
                                        res.status(400)
                                            .json({
                                                ppResult: 'err',
                                                ppMsg: err.ppMsg ? err.ppMsg : "创建邀请失败!",
                                                err: err
                                            });
                                    } else {
                                        res.json({
...
router.post('/getSpecialInfo', function (req, res) {
    if (req.user.specialInfo.hair) {
        res.json({
            ppResult: 'ok',
            ppData: {
                specialInfo: req.user.specialInfo,
...
router.post('/uploadSpecialPic', function (req, res) {
    if (!(req.files && req.files.specialPic)) {
        ...
        res.json({
            ppResult: 'ok',
            ppData: req.files.specialPic.name
        });
    }
});
